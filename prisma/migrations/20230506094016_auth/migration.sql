/*
  ...
  - The primary key for the `Deposit_wallet` table will be changed. If it partially fails, the table could be left without primary key constraint.
  ...
  - A unique constraint covering the columns `[userId]` on the table `Deposit_wallet` will be added. If there are existing duplicate values, this will fail.
  ...
  - Changed the type of `user_id` on the `Deposit_wallet` table. No cast exists, the column would be dropped and recreated, which cannot be done if there is data, since the column is required.
...
-- AlterTable
ALTER TABLE "Deposit_wallet" DROP CONSTRAINT "Deposit_wallet_pkey",
DROP COLUMN "user_id",
